...
class Body:
    ...
    def find_center(self):
        max_x = max([vertex[0] for facet in self.body for vertex in facet])
        min_x = min([vertex[0] for facet in self.body for vertex in facet])
        
        max_y = max([vertex[1] for facet in self.body for vertex in facet])
        min_y = min([vertex[1] for facet in self.body for vertex in facet])
        
        max_z = max([vertex[2] for facet in self.body for vertex in facet])
        min_z = min([vertex[2] for facet in self.body for vertex in facet])
    ...
    def find_size(self):
        size = (abs(max([vertex[0] for facet in self.body for vertex in facet]) - min([vertex[0] for facet in self.body for vertex in facet])),
                abs(max([vertex[1] for facet in self.body for vertex in facet]) - min([vertex[1] for facet in self.body for vertex in facet])),
                abs(max([vertex[2] for facet in self.body for vertex in facet]) - min([vertex[2] for facet in self.body for vertex in facet])))
        return size
...
class Screen:
    def __init__(self, body_center, body_size):
        ...
        self.display_size = [1200, 675]
        self.screen_to_body_ratio = 0.8
        self.line_size = 1
        self.y_distance_multiplier = 5
    ...
    def add_stats(self, img, msg):
        draw = ImageDraw.Draw(img)
        draw.text((10, 10), msg, (255,255,255))
        return img
    ...
    def generate_img(self, body, cam):
        ...
        for facet in body.rotated_body:
            ...
            if len(corners) == 3:
                draw.line([corners[0], corners[1]], fill=(255,255,255), width=self.line_size)
                draw.line([corners[1], corners[2]], fill=(255,255,255), width=self.line_size)
                draw.line([corners[2], corners[0]], fill=(255,255,255), width=self.line_size)
...
# body.rotated_body = body.rotate((0,45,0))
...
while True:
    for r in range(720):
        body.rotated_body = body.rotate((r/2, r/2, r/2))

        img = screen.generate_img(body, cam)
        img  = screen.add_stats(img, f"Rotation: {r/2}")
